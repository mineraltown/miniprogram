...
Component({
    ...
    created() {
        ...
        // 读档
        if (!wx.getStorageSync("mineraltown")) {
            console.log("初始化")
            wx.setStorageSync("mineraltown", {
                year: 1,
                month: 0,
                day: 1,
                birthday_month: 0,
            ...
            var d = wx.getStorageSync("mineraltown")
            this.setData({
                year: d.year,
                month: d.month,
                day: d.day,
        ...
        wx.request({
            url: app.globalData.url + "todo/mineraltown/resident",
            header: {
        ...
        wx.request({
            url: app.globalData.url + "todo/mineraltown/festival",
            header: {
                'content-type': 'application/json'
            },
            success(res) {
    ...
    methods: {
        ...
        get_calendar() {
            var ThisWeek = []
            var today = ((this.data.year - 1) * 120 + this.data.month * 30 + this.data.day - 1) % 7
            // 当天之前（不含当天）
            for (var i = today; i > 0; i--) {
                var d = this.data.day - i
                if (d <= 0) {
                    ThisWeek.push([this.data.month - 1, 30 + d, this.data.year])
                } else {
            ...
            for (var i = today; i < 7; i++) {
                ...
                if (d > 30) {
                    ThisWeek.push([this.data.month + 1, d - 30, this.data.year])
                } else {
                    ThisWeek.push([this.data.month, d, this.data.year])
                }
            }
